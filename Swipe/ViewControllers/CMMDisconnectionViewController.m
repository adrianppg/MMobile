//
//  CMMDisconnectionViewController.m
//  CMMobile
//
...
#import "CMMDisconnectionViewController.h"
#import "CMMSwipe.h"
#import "CMMLogic.h"

@implementation CMMDisconnectionViewController
...
- (void)_showSocket:(CMMSwipeDirection)direction
{
    ...
    if (direction == CMMSwipeDirectionBottomUp) {
        ...
        _socketImageView.frame = CGRectMake((windowSize.width - height)/2, windowSize.height - width, height, width);
        _pinsImageView.transform = CGAffineTransformMakeRotation(M_PI + M_PI/2);
        _pinsImageView.frame = CGRectMake((windowSize.width - pinsHeight)/2, windowSize.height - pinsWidth, pinsHeight, pinsWidth);
    }
    else if (direction == CMMCSwipeDirectionLeftToRight) {
        _socketImageView.frame = CGRectMake(0, (windowSize.height - height)/2, width, height);
        _pinsImageView.frame = CGRectMake(0, (windowSize.height - pinsHeight)/2, pinsWidth, pinsHeight);
    }
    
    _socketImageView.hidden = NO;
    _socketImageView.alpha = 0;
    _pinsImageView.hidden = NO;
    _pinsImageView.alpha = 0;
    
    [UIView animateWithDuration:START_ANIMATION_DURATION delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        _pinsImageView.alpha = 1;
        _socketImageView.alpha = 1;
    }  completion:^(BOOL finished) {
        [self _animateSocket];
    }];
}
...
- (void)_animateSocket
{
    ...
    if (center.y == windowSize.height / 2) {
        duration = END_ANIMATION_DURATION / 3.5;
    }
    
    [UIView animateWithDuration:duration delay:ANIMATION_PAUSE_DURATION options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (center.x == windowSize.width / 2) {
            ...
            pinsFrame.origin.x = -pinsFrame.size.width;
        }
    ...
    [UIView animateWithDuration:END_ANIMATION_DURATION delay:ANIMATION_PAUSE_DURATION options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (center.x == windowSize.width / 2) {
            ...
            frame.origin.x = windowSize.width;
        }
...
- (void)_showPlugAndPins:(CMMSwipeDirection)direction
{
    ...
    if (direction == CMMSwipeDirectionTopDown) {
        _pinsImageView.transform = CGAffineTransformMakeRotation(M_PI + M_PI/2);
        _pinsImageView.frame = CGRectMake((windowSize.width - pinsHeight)/2, -pinsWidth, pinsHeight, pinsWidth);
        
        _plugImageView.transform = CGAffineTransformMakeRotation(M_PI + M_PI/2);
        _plugImageView.frame = CGRectMake((windowSize.width - plugHeight)/2, 0, plugHeight, plugWidth);
        
    }
    else if (direction == CMMSwipeDirectionRightToLeft) {
        _pinsImageView.frame = CGRectMake(windowSize.width, (windowSize.height - pinsHeight)/2, pinsWidth, pinsHeight);
        _plugImageView.frame = CGRectMake(windowSize.width - plugWidth, (windowSize.height - pinsHeight)/2, plugWidth, plugHeight);
    }
    
    _pinsImageView.hidden = NO;
    _pinsImageView.alpha = 0;
    _plugImageView.hidden = NO;
    _plugImageView.alpha = 0;
    
    [UIView animateWithDuration:START_ANIMATION_DURATION delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        _pinsImageView.alpha = 1;
        _plugImageView.alpha = 1;
    }  completion:^(BOOL finished) {
...
- (void)_animatePinsAndPlug
{
    ...
    [UIView animateWithDuration:END_ANIMATION_DURATION delay:ANIMATION_PAUSE_DURATION options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (center.x == windowSize.width / 2) {
            pinFrame.origin.y = windowSize.height;
            plugFrame.origin.y = windowSize.height + pinFrame.size.height;
        }
        else if (center.y == windowSize.height / 2) {
            pinFrame.origin.x = - pinFrame.size.width;
            plugFrame.origin.x = - pinFrame.size.width - plugFrame.size.width;
        }
